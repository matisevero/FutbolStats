import type { Match, HistoricalRecords, HistoricalRecord, CustomAchievement } from '../types';
import { MoraleLevel } from '../types';

/**
 * Parses a 'YYYY-MM-DD' date string as a local date, avoiding timezone issues where
 * `new Date('YYYY-MM-DD')` can be interpreted as UTC midnight and result in the previous day.
...
export const calculatePlayerMorale = (matches: Match[]): { level: MoraleLevel; score: number; recentMatchesSummary: any; trend: 'up' | 'down' | 'same' | 'new' } | null => {
  const sortedMatches = [...matches].sort((a, b) => parseLocalDate(b.date).getTime() - parseLocalDate(a.date).getTime());
  ...
  return { level, score: currentScore, recentMatchesSummary, trend };
};