...
import { MoraleLevel, type TeammateStats, type OpponentStats } from '../types';
...
const SocialPage: React.FC = () => {
  ...
  const shareableMoments: ShareableMoment[] = useMemo(() => {
    ...
    // NEW: Yearly Summary
    const yearsWithMatches = Array.from(new Set(matches.map(m => parseLocalDate(m.date).getFullYear()))).sort((a,b) => b-a);
    if (yearsWithMatches.length > 0) {
        const latestYear = yearsWithMatches[0];
        const yearlyMatches = matches.filter(m => parseLocalDate(m.date).getFullYear() === latestYear);
    ...
    if (matches.length > 0) {
        ...
        if (topPerformanceMatch && (!recentWin || topPerformanceMatch.id !== recentWin.id)) {
            moments.push({
                ...
                date: parseLocalDate(topPerformanceMatch.date).toLocaleDateString('es-ES', { day: 'numeric', month: 'short' }),
    ...
    return moments;
  }, [matches, playerProfile.name, theme.colors.accent1]);
